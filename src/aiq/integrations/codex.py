...
def receive_hook_main(
    *,
    ...
    work remains and ``stop_hook_active`` is falsy, exit 0 with one
    stderr notice surfaces parked ``needs_input`` messages when nothing
    is runnable, exit 0 is silent otherwise, and any gate failure exits
    0 with a single diagnostic so an AIQ defect never blocks stopping.
    """
